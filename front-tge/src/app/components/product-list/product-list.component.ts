...
export class ProductListComponent {
  ...
  public getMaxValue() {
    this.productService.getProducts()
      .pipe(map((products: Product[]) => this.locationFalse(products))
      )
      .subscribe(
        (data: any) => {
 ...
 */  public categorieSelected(event: Event, id: number) {
  ...
  brandsSelected(event: Event, brand: string) {
    const isChecked = (event.target as HTMLInputElement).checked;
    if (isChecked) {
      /* this.brandSelected.push(brand) // pas la méme structure avec product.brand */
  ...
  filterProductsByBrand(products: Product[]): Product[] {
    if (this.brandSelected.length > 0) {
      return products.filter(product => this.brandSelected.includes(product.brand));
    }
    else {
      return products;
    }
  ...
  /* selection by puissances , 
    ...
    filterProductsByCapotage(products: Product[]): Product[] {
      if (this.capotageSelected.length > 0) {
